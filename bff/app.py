    ...
    try:
        ...
        for line in m.splitlines():
            if line.startswith("iip_integrity_score "):
                ...
                break
        resp = jsonify(score=score, source="webhook-listener")
        code = 200
    except Exception as e:
        resp = jsonify(error=str(e))
        code = 502
    observe("/v1/integrity", "GET", code, t)
    return resp, code
...
def intake():
    t = time.time()
    resp = jsonify(id="stub-001", data=request.get_json(silent=True))
    code = 201
    observe("/v1/intake", "POST", code, t)
    return resp, code
